...
import { StyleSheet, Text, View,TouchableOpacity,StatusBar,Platform,FlatList,Image,ScrollView,RefreshControl,ActivityIndicator} from 'react-native'
...
export default class ResultatsNatation extends React.Component{
    ...
    chargerEcoles(){
        ...
        var docRef=db.collection("Statistiques").doc('Natation').collection('Ecoles')
        ...
        docRef.get().then(function(querySnapshot) {
            ...
            that.setState({listeEquipes:tab})
    ...
    trierEcoles(){
        var listeEquipes=this.state.listeEquipes
        var tab=[]
        for (i=0;i<listeEquipes.length;i++){
            if (listeEquipes[i].classement>=1){
                tab.push(listeEquipes[i])
            }
        ...
        this.setState({listeEquipes,refreshing:false,loading:false})
    ...
    imageEcole(idEcole){
        switch (idEcole){
            ...
            break;
            case 108:
            return(require('../../../../Images/LogoEcoles/LogoEcole_108.png'))
            break;
            default:
    ...
    displayLoading(){
        if (this.state.loading){
            return(<View style={{justifyContent:'center',alignItems:'center',flex:1}}><ActivityIndicator/></View>)
        }
        else{
            if (this.state.listeEquipes.length===0){
                return(
                <View style={{flex:1,justifyContent:'center',alignItems:'center',marginHorizontal:20}}><Text style={{fontWeight:'bold',fontSize:20,textAlign:'center'}}>Les résultats seront disponibles à la fin de l'épreuve</Text></View>
                )
            }
            else{
                return(
            ...
            <ScrollView refreshControl={
                    ...
                    />}>
            <FlatList extraData={this.state} data={this.state.listeEquipes} keyExtractor={(item)=>item.equipe.toString()}
                        renderItem={({item,index})=>{
                            return(
                                <View  style={{height :55, flexDirection: 'row',backgroundColor:this.choixStyle({index}),borderColor:'#EEEFF0',borderBottomWidth:1}}>
                                    <View style={{flex:6,flexDirection:'row'}}>
                                        <View style={{ flex: 1, alignItems: 'center',flexDirection:'row'}}>
                                            <Text style={{fontWeight:'bold', marginLeft:15, fontSize:17}}>{item.classement}</Text>
                                            <Image source={this.imageEcole(item.id)} style={{height:30,width:30,marginLeft:20,resizeMode:'contain'}}/>
                                        </View>
                                        <View style={{flexDirection:'row',alignItems:'center',flex:3.5,paddingRight:5,paddingLeft:5,justifyContent:'space-between'}}>
                                        
                                            <Text style={{fontWeight:'bold',marginLeft:5}} >{item.equipe}</Text>
                                            {
                                                item.classement===1?<Image style={{height:30,width:30,marginRight:10}} source={require('../../../../Images/Medailles/gold-medal.png')}/>
                                                : null
                                            }
                                            {
                                                item.classement===2?<Image style={{height:30,width:30,marginRight:10}} source={require('../../../../Images/Medailles/silver-medal.png')}/>
                                                : null
                                            }
                                            {
                                                item.classement===3?<Image style={{height:30,width:30,marginRight:10}} source={require('../../../../Images/Medailles/bronze-medal.png')}/>
                                                : null
                                            }
    ...
    render() {
            ...
            <View>
              ...
              <View style={{height:50,flexDirection:'row',backgroundColor:'#070027'}}>
                ...
                <View style={{flex:3,justifyContent:'center',alignItems:'center'}}>
                  <Text style={{color:'white',fontWeight:'bold',fontSize:18}}>Résultats Natation</Text>
                </View>
                <View style={{flex:1}}>
                </View>
              </View>
        ...
        :
         ...
         <Header transparent style={{backgroundColor:'#070027',height:65}}>
          ...
          <Body style={{flex:5}}>
            <Title style={{color:'white'}}>Résultats Natation</Title>
          </Body>
          <Right style={{flex:1}}/>  
        </Header> 
        }
