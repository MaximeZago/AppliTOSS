...
import {Text, View,FlatList,Image,ScrollView,RefreshControl,ActivityIndicator} from 'react-native'
...
export default class ResultatsQualifsF extends React.Component{
    
    constructor(props){
        super(props)
        this.state={listeParticipants:[],refreshing:false,loading:true}
    }
    ...
    chargerParticipants(){
        ...
        var docRef=db.collection("Statistiques").doc('DiffF').collection('Participants').where("phase","==","qualif")
        ...
        docRef.get().then(function(querySnapshot) {
            ...
            that.setState({listeParticipants:tab})
            
        })
        .then(()=>this.trierParticipants())
        
    }

    trierParticipants(){
        var listeParticipants=this.state.listeParticipants
        var tab=[]
        for (i=0;i<listeParticipants.length;i++){
            tab.push(listeParticipants[i])
        }
        listeParticipants=tab
        listeParticipants.sort(function(a,b){
            return(a.classement-b.classement)
        })
        this.setState({listeParticipants,refreshing:false,loading:false})
    }
    ...
    arrondie(number){
        n=Number(number)
        if ((n-Math.round(n))>(Math.round(n)+1>n)){
    ...
    render() {
        ...
        return (
            ...
            {this.state.loading?<View style={{flex:1,justifyContent:'center',alignItems:'center'}}><ActivityIndicator/></View>:
                ...
                <ScrollView refreshControl={
                        ...
                        />}>
                <FlatList extraData={this.state} data={this.state.listeParticipants} keyExtractor={(item)=>item.nom.toString()}
                                ...
                                return(
                                    <View  style={{height :55, flexDirection: 'row',backgroundColor:this.choixStyle({index}),borderColor:'#EEEFF0',borderBottomWidth:1}}>
                                            <View style={{ flex: 1, alignItems: 'center',flexDirection:'row'}}>
                                                ...
                                                <Image source={this.imageEcole(item.idEcole)} style={{height:30,width:30,marginLeft:20,resizeMode:'contain'}}/>
                                            </View>
                                            <View style={{alignItems:'flex-start',justifyContent:'center',flex:3.5,paddingRight:5,paddingLeft:5,marginLeft:15}}>
                                                <Text style={{fontWeight:'bold'}} >{item.prenom} {item.nom}</Text>
                                            </View>
                                            <View style={{justifyContent:'center',alignItems:'flex-end',flex:1}}>
                                                <Text style={{marginRight:15}}>{this.arrondie(item.points)}</Text>
                                            </View>
                                    </View>
                                )
                            }}
