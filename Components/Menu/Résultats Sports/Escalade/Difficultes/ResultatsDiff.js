...
import ResultatsDiffM from './Masculin/ResultatsDiffM'
import ResultatsDiffF from './Feminin/ResultatsDiffF'

export default class ResultatsDiff extends React.Component {
  ...
  displayVue () {
    if (this.state.masculin) {      
      return(<ResultatsDiffM/>) 
    }
    else {
      return(<ResultatsDiffF/>)    }
  }
      ...
      <Container>
        {Platform.OS==='android'?
            <View>
              ...
              <View style={{height:50,flexDirection:'row',backgroundColor:'#070027'}}>
                <View style={{flex:1}}>
                  <TouchableOpacity onPress={()=>this.props.navigation.navigate('ResultatEscalade')} style={{flex:1,justifyContent:'center'}}>
                      <Icon style={{marginLeft:15,marginRight:20,color:'white'}}   name='arrow-back'/>
                  </TouchableOpacity>
                </View>
                <View style={{flex:3,justifyContent:'center',alignItems:'center'}}>
                  <Text style={{color:'white',fontWeight:'bold',fontSize:18}}>Difficulté</Text>
                </View>
                <View style={{flex:1}}>
                </View>
              </View>
        ...
        :
         ...
         <Header transparent style={{backgroundColor:'#070027',height:65}}>
          ...
          <Body style={{flex:5}}>
            <Title style={{color:'white'}}>Difficulté</Title>
          </Body>
          <Right style={{flex:1}}/>  
        </Header> 
        }
