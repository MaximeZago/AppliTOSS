...
import {View, Text,TouchableOpacity,ScrollView,RefreshControl,StyleSheet,TextInput,FlatList,Image,StatusBar,Dimensions,ActivityIndicator} from 'react-native'
...
export default class Informations extends React.Component{
    ...
    constructor(props){
        super(props)
        this.state={donnees:[],listeOr:[],listeArgent:[],listeBronze:[],
        loading:true
        }
    }
    ...
    chargerEcoles(){
        ...
        var docRef=db.collection("Classement").where("idEcole",'==',this.props.idEcole)
        ...
        docRef.get().then(function(querySnapshot) {
            ...
            that.setState({donnees:tab,loading:false,listeOr:tab[0].listeOr,listeArgent:tab[0].listeArgent,listeBronze:tab[0].listeBronze})
            ...
            {this.state.loading?
                ...
                <View style={{height:150,flexDirection:'row'}}>
                    <View style={{flex:1,justifyContent:'center',alignItems:'center'}}>
                        ...
                        <Image style={{height:100,width:100,resizeMode:'contain'}} source={this.imageEcole(this.props.idEcole)}/>
                    </View>

                    <View style={{flex:2,flexDirection:'column',justifyContent:'center',marginLeft:5}}>
                        <View style={{flexDirection:'row',alignItems:'center'}}>
                            <Text style={{fontWeight:'bold',fontSize:18,marginVertical:4}}>Classement général :  </Text>
                            <Text style={{fontWeight:'bold',fontSize:22,color:'#D23D53'}}>{this.state.donnees[0].classement}</Text>
                        </View>
                        {this.state.donnees[0].nombreParticipants===1?
                        <Text style={{fontWeight:'bold',fontSize:18,marginVertical:4}}>{this.state.donnees[0].nombreParticipants}  participant</Text>
                        :
                        <Text style={{fontWeight:'bold',fontSize:18,marginVertical:4}}>{this.state.donnees[0].nombreParticipants}  participants</Text>
                        }
                        {this.state.donnees[0].nombresports===1?
                        <Text style={{fontWeight:'bold',fontSize:18,marginVertical:4}}>{this.state.donnees[0].nombresports}  sport représenté</Text>
                        :
                        <Text style={{fontWeight:'bold',fontSize:18,marginVertical:4}}>{this.state.donnees[0].nombresports}  sports représentés</Text>
                        }
